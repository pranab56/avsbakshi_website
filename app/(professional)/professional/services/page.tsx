...
import { Plus, Check, Edit2, Power, Upload } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div>
          <button
            ...
            className="px-4 py-2.5 rounded-sm bg-primary hover:bg-primary/90 text-primary-foreground text-xs sm:text-sm font-medium transition-colors cursor-pointer flex items-center gap-2 shadow-xs active:scale-[0.98]"
          >
          ...
          >
            ...
            <div className="flex items-start gap-4 max-w-xl">
              ...
              <div className="space-y-1">
                ...
                <div className="pt-2">
                  <span className="px-3 py-1 bg-accent border border-border text-muted-foreground text-xs font-medium rounded-full">
                    {service.duration}
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4 sm:gap-6 justify-between sm:justify-end shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-border">
              ...
              <button
                ...
                className="px-4 py-2 rounded-sm bg-accent hover:bg-accent/80 text-foreground text-xs font-medium transition-colors cursor-pointer flex items-center gap-1.5"
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-sm text-xs font-medium transition-colors cursor-pointer flex items-center gap-1.5 ${
                  service.active
                    ? "border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-500"
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-xl bg-card text-card-foreground border border-border p-6 sm:p-8 rounded-lg shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 pt-2">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-accent/50 border border-border rounded-sm px-4 py-3 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full bg-accent/50 border border-border rounded-sm px-4 py-3 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground resize-none"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {imagePreview ? (
                <div className="relative w-full h-36 rounded-lg overflow-hidden border border-border bg-accent/50 group">
                  <Image
                    src={imagePreview}
                    alt="Service Preview"
                    fill
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-card hover:bg-accent text-foreground text-xs font-medium rounded-md shadow-xs transition-colors cursor-pointer"
                    >
                      Change
                    </button>
                    <button
                      ...
                      className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-md shadow-xs transition-colors cursor-pointer"
                    >
                      Remove
                    </button>
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full h-28 border-2 border-dashed border-border hover:border-primary bg-accent/30 rounded-lg flex flex-col items-center justify-center gap-1.5 transition-colors cursor-pointer group"
                >
                  <div className="w-9 h-9 rounded-full bg-accent text-muted-foreground group-hover:bg-primary/15 group-hover:text-primary flex items-center justify-center transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Combobox
                  ...
                  triggerClassName="w-full h-11 bg-accent/50 border-border rounded-sm px-4 text-sm font-medium text-foreground"
                  align="start"
                />
              </div>
            ...
            <div className="pt-4 flex items-center gap-3 border-t border-border">
              <button
                type="submit"
                className="px-6 py-2.5 rounded-sm bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium transition-colors cursor-pointer shadow-xs active:scale-[0.98] flex items-center gap-2"
              >
                {savedSuccess ? (
                  <>
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-sm bg-secondary border border-border text-secondary-foreground hover:bg-secondary/80 text-sm font-medium transition-colors cursor-pointer"
              >
                Cancel
              </button>
            </div>
