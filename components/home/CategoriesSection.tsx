      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {categories.map((cat, idx) => (
          <Link
            ...
            className="group relative h-48 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-[#E5E0D6]"
          >
            <div
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
              <div>
                <h3 className="font-title text-lg font-bold text-white group-hover:text-[#CAA054] transition-colors">
                  {cat.title}
                </h3>
                <p className="text-[11px] text-zinc-300 font-light">{cat.count}</p>
              </div>
              <span className="w-7 h-7 rounded-sm bg-white/20 text-white flex items-center justify-center backdrop-blur-sm group-hover:bg-[#B78735] transition-colors">
